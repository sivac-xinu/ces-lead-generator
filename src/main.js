...
function leadToDbRow(lead) {
  return {
    ...
    icp: lead.icp || '',
    tier: lead.tier || '',
    imported_by: lead.imported_by || null,
...
async function aiAnalyze(lead) {
  if (aiConfig.provider === 'openrouter' && aiConfig.apiKey) {
    try {
      ...
      if (!parsed.enrichment) parsed.enrichment = null;
      return parsed;
...
async function runIntelligence(leadId) {
  ...
  document.getElementById('intel-depth').textContent = 'Deep Research';
  const srcLine = document.getElementById('intel-source-line');
  if (lead.imported_by) {
    srcLine.textContent = 'Imported by ' + lead.imported_by;
    srcLine.style.display = 'block';
  } else {
    srcLine.style.display = 'none';
  }
  document.getElementById('intel-enrichment').style.display = 'none';
  document.getElementById('intel-research').style.display = 'none';
  ...
  if (!result) result = deepInferAll(lead);
  intelResult = result;
  ...
  if (result.enrichment && result.enrichment.company_context) {
  ...
  if (result.research && result.research.recent_activities) {
...
window.openEditIcpTier = openEditIcpTier;
window.openEditPainPoints = openEditPainPoints;
...
window.removeApolloTitle = removeApolloTitle;
window.removePPChip = removePPChip;
...
window.runIntelligence = runIntelligence;
...
window.selectIntelICP = selectIntelICP;
